...
### 6.4 Twitch OAuth トークンのバックグラウンド自動ライフサイクル管理
* **設計内容**:
  - 長時間のゲーム配信などにおけるトークンの自然期限切れ（4時間）を防ぎ、ユーザーの手動再認証を完全に不要とするため、**能動的（プロアクティブ）なバックグラウンド自動リフレッシュ機能**を適用する。
  - `Application` のバックグラウンドで稼働する `QTimer` を配置し、2時間（7200秒）おきにリフレッシュトークンを使用してアクセストークンを自動的に最新にローテーションする。
  - アプリの起動時にも、保存されているリフレッシュトークンがある場合は即座にバックグラウンドでリフレッシュを実行してから EventSub への接続を行う。これにより、アプリ起動時から常に新鮮なアクセストークンが保証される。
  - **エラーのインテリジェント判別ロジック**:
    - リフレッシュ時に失敗した場合、エラーの起因が「一時的なネットワーク断線（タイムアウトなど）」であるか、「恒久的な設定ミス（不正なクライアント情報やトークン無効化）」であるかを判別する。
    - **一時的エラー**の場合は、配信を邪魔しないよう警告を非表示にし、5分後にバックグラウンドで自動リトライを行う。ただし無限リトライは行わず、最大5回（合計25分間）連続して失敗した場合はリトライを停止し、エラー状態をログおよびステータスに記録する。
    - **恒久的エラー**の場合は、無駄なリクエストの繰り返しを防ぐためにタイマーを停止し、UI経由でユーザーに警告を通知してシステム設定からの再認証を促す。

### 6.5 多言語化（国際化：i18n）設計
* **設計内容**:
  - グローバルな配信者利用に対応するため、GUI画面およびダイアログメッセージ全般（ログメッセージを除く）を5つの主要言語（英語、フランス語、ドイツ語、ポルトガル語、スペイン語）に対応させる多言語化を導入する。
  - **動的ロケール自動検出および手動設定 (再起動不要)**:
    - アプリ起動時（`main.cpp` / `db_viewer_main.cpp`）に、ユーザーのOS（Windows）のシステム言語（ロケール）を自動検出します。
    - 検出したロケールが対応言語（`en` / `fr` / `de` / `pt` / `es`）であれば、自動的にその言語用の翻訳データをロードします。それ以外の言語（または日本語）の場合はデフォルトの日本語が適用されます。
    - さらに、システム設定（`SettingsWidget`）に「表示言語設定」グループボックスを設け、ユーザーが手動で言語を強制指定（Deutsch, English, Español, Français, Português, 日本語, もしくはシステムデフォルト）できるようにします。
    - 手動設定のプルダウンは、アルファベット昇順（Deutsch, English, Español, Français, Português, 日本語）でソートして提供されます。
    - **再起動不要設計**: 言語変更を選択・保存したその瞬間に、アプリの再起動を伴わずにGUI全体が即座に指定言語に切り替わります（動的翻訳）。設定は `config.json` に保存され、次回以降の起動時にも適用されます。
  - **リソース埋め込み方式の採用**:
    - 翻訳用のバイナリファイル（`.qm`）を外部ファイルとして配布するのではなく、Qtリソースシステム（`.qrc`）を利用してアプリの実行ファイル内に直接埋め込む。これにより、ファイルの紛失や不整合を完全に防止し、配布バイナリを「全世界で完全に同一の1つのファイル」に集約する。
  - **レビュー用多言語対応表 (CSV) の自動出力**:
    - 翻訳完了後、ネイティブレビュアーによる見直し（レビュー）を容易にするため、日本語を左列に配した5言語の対比CSVファイル（`translation_review.csv`）をプロジェクトルートへ自動生成する。
    - CSVファイルは UTF-8 BOM付きで生成され、Excel等で文字化けせずそのまま閲覧・編集可能とする。また、Git管理に誤って登録されないよう、`.gitignore` の追跡対象から完全に除外する。
```
